"""
Pre-clustering training script.
Requires data in 'Traindata_NanoML2' format
Run it like:
$ python3 pre_snowflake.py /path/to/data/dataCollection.djcdc /path/to/trained/model
"""
...
from model_blocks import double_tree_condensation_block
...
# TODO: There should be only one 'LLExtendedObjectCondensation'
LOSS_OPTIONS = {
    "beta_loss_scale": 1.0,
...
def config_model(
    ...
    pre_processed = condition_input(
        orig_input, no_scaling=True, no_prime=False, new_prime=True
    )
    ...
    out, graph, sels, graph0 = double_tree_condensation_block(
        pre_processed,
    ...
    out["prime_coords"] = PlotCoordinates(
        ...
        publish="wandb",
        name="output_coords",
    )([out["prime_coords"], out["rechit_energy"], out["t_idx"], out["row_splits"]])
    ...
    output = dict()
    for key, value in graph0.items():
        output[f"g0_{key}"] = value
