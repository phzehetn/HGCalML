print(
    ...
    "move layers still in usage or restructure all the layer files",
)
...
class RaggedSumAndScatter(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        ragged = tf.RaggedTensor.from_row_splits(values=data, row_splits=rs)
...
class Condensate(tf.keras.layers.Layer):
    def __init__(self, t_d, t_b, soft, feature_length, **kwargs):
        print(
            "Condensate Layer: warning, do not use this in the model, a bunch of hardcoded stuff!"
        )
        self.t_d = t_d
        self.t_b = t_b
        self.soft = soft
    ...
    def call(self, inputs):
        ...
        _, asso, _, is_cond, ncond = BuildAndAssignCondensatesBinned(
            ccoords, betas, d, row_splits=row_splits, beta_threshold=self.t_b
        )
        ...
        dout = x[iscond > 0]
        dout = tf.reshape(
            dout, [-1, dout.shape[-1]], name="predicted_final_condensates"
        )
...
class CondensateToPseudoRS(keras.layers.Layer):
    ...
    def get_config(self):
        config = {"radius": self.radius, "threshold": self.threshold, "soft": self.soft}
    ...
    def call(self, inputs):
        data, ccoords, betas, row_splits = (
            inputs[0],
            inputs[1],
            inputs[2],
            inputs[3],
        )
        asso_idx, is_cpoint, n = BuildCondensates(
            ccoords,
            betas,
            row_splits,
            radius=self.radius,
            min_beta=self.threshold,
            soft=self.soft,
        )
...
class RaggedGlobalExchange(keras.layers.Layer):
    def __init__(self, skip_min=False, **kwargs):
        """
        Inputs:
        - data
        - row splits
    ...
    def call(self, x):
        ...
        rt = tf.RaggedTensor.from_row_splits(
            values=x_data, row_splits=x_row_splits
        )  # [B, {V}, F]
        means = tf.reduce_mean(rt, axis=1)  # [B, F]
        rmax = tf.reduce_max(rt, axis=1)  # [B, F]
        data_means = tf.gather_nd(
            means, tf.ragged.row_splits_to_segment_ids(rt.row_splits)[..., tf.newaxis]
        )  # [SV, F]
        data_max = tf.gather_nd(
            rmax, tf.ragged.row_splits_to_segment_ids(rt.row_splits)[..., tf.newaxis]
        )  # [SV, F]

        if not self.skip_min:
            rmin = tf.reduce_min(rt, axis=1)  # [B, F]
            data_min = tf.gather_nd(
                rmin,
                tf.ragged.row_splits_to_segment_ids(rt.row_splits)[..., tf.newaxis],
            )  # [SV, F]
